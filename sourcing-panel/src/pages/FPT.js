...
export default function FPT() {
  const [reports, setReports] = useState([]);
  const [selectedReport, setSelectedReport] = useState(null);
  const [modalOpen, setModalOpen] = useState(false);

  useEffect(() => {
    fetch('http://localhost:3001/api/fptreports')
      .then(res => res.json())
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-4">FPT Report</h1>
      <div className="bg-white rounded shadow p-4">
        <table className="min-w-full">
          <thead>
            <tr>
              <th className="text-left p-2">FPT REPORT ID</th>
          ...
          <tbody>
            {reports.map((report) => (
              <tr key={report._id} className="hover:bg-gray-100 cursor-pointer" onClick={() => handleRowClick(report)}>
                <td className="p-2">FPT-{report.createdAt ? new Date(report.createdAt).toISOString().slice(0,10).replace(/-/g,"") : ''}-001</td>
      ...
      {modalOpen && selectedReport && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-2xl relative">
            ...
            <h2 className="text-xl font-bold mb-4">FPT Report Details</h2>
