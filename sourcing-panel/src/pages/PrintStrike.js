...
const CONSOLIDATED_DATA = [
  {
    ...
    printStrikeUpNoOfOptions: 400, printStrikeUpApproved: 200, printStrikeUpApprovalPending: 200, printStrikeUpApprovalPercent: 50, viewStyleLevelStatus: ''
  },
  {
    ...
    printStrikeUpNoOfOptions: 100, printStrikeUpApproved: 25, printStrikeUpApprovalPending: 75, printStrikeUpApprovalPercent: 25, viewStyleLevelStatus: ''
  },
  {
    ...
    printStrikeUpNoOfOptions: 200, printStrikeUpApproved: 150, printStrikeUpApprovalPending: 50, printStrikeUpApprovalPercent: 75, viewStyleLevelStatus: ''
  },
  {
    sourcingSpoc: 'Ravi', vendorName: 'Sonam Exports', statusDate: '20-06-2025', brand: 'xyz', articleType: 'Tops', gender: 'Women', season: 'SS26',
    printStrikeUpNoOfOptions: 300, printStrikeUpApproved: 200, printStrikeUpApprovalPending: 100, printStrikeUpApprovalPercent: 67, viewStyleLevelStatus: ''
  },
  {
    sourcingSpoc: 'Ravi', vendorName: 'Madhu Garments', statusDate: '20-06-2025', brand: 'xyz', articleType: 'Jackets', gender: 'Men', season: 'SS26',
    printStrikeUpNoOfOptions: 20, printStrikeUpApproved: 5, printStrikeUpApprovalPending: 15, printStrikeUpApprovalPercent: 25, viewStyleLevelStatus: ''
  },
];
...
const PrintStrike = () => {
  ...
  // Manager card search/sort hooks (must be top-level)
  const [managerSearch, setManagerSearch] = useState('');
  const [managerSortAsc, setManagerSortAsc] = useState(true);

  useEffect(() => {
    setLoading(true);
    fetch('http://localhost:3001/api/printstrike')
      .then(res => res.json())
      .then(data => {
  ...
  if (showConsolidated) {
    ...
    const downloadExcel = () => {
      const headers = [
        ...
        'Print Strike Up No of Options required', 'Print strike up Approved', 'Print strike up Approval pending', 'Print strike up approval %', 'View Style level Status'
      ];
      const rows = CONSOLIDATED_DATA.map(row => [
        ...
        row.printStrikeUpNoOfOptions, row.printStrikeUpApproved, row.printStrikeUpApprovalPending, row.printStrikeUpApprovalPercent, row.viewStyleLevelStatus
      ]);
      let csvContent = '';
      ...
      a.href = url;
      a.download = 'print_strike_status.csv';
      ...
      <div style={{ background: '#F7F8FA', minHeight: '100vh', padding: 0, paddingTop: 80 }}>
        <div style={{ maxWidth: 1300, margin: '0 auto', padding: '32px 0 0 0', position: 'relative' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            <h1 style={{ fontSize: 28, fontWeight: 700, margin: 0 }}>Print Strike Up Status</h1>
            <button
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15 }}>
              <thead>
                <tr style={{ background: '#F3F4F6' }}>
                  ...
                  <th style={{ padding: 8, border: '1px solid #E5E7EB' }}>Print Strike Up No of Options required</th>
                  <th style={{ padding: 8, border: '1px solid #E5E7EB' }}>Print strike up Approved</th>
                  <th style={{ padding: 8, border: '1px solid #E5E7EB' }}>Print strike up Approval pending</th>
                  <th style={{ padding: 8, border: '1px solid #E5E7EB' }}>Print strike up approval %</th>
              ...
              <tbody>
                {CONSOLIDATED_DATA.map((row, idx) => (
                  <tr key={idx}>
                    ...
                    <td style={{ padding: 8, border: '1px solid #E5E7EB' }}>{row.printStrikeUpNoOfOptions}</td>
                    <td style={{ padding: 8, border: '1px solid #E5E7EB' }}>{row.printStrikeUpApproved}</td>
                    <td style={{ padding: 8, border: '1px solid #E5E7EB' }}>{row.printStrikeUpApprovalPending}</td>
                    <td style={{ padding: 8, border: '1px solid #E5E7EB' }}>{row.printStrikeUpApprovalPercent}</td>
  ...
  if (selectedManager && managerRecords && managerRecords.length > 0) {
    ...
    const showMore = filtered.length > 10;
    const visible = filtered.slice(0, 10);

    return (
      <div style={{ background: '#F7F8FA', minHeight: '100vh', padding: 0, position: 'relative', paddingTop: 80 }}>
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '32px 0 0 0', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
            <h1 style={{ fontSize: 28, fontWeight: 700, margin: 0 }}>Print Strikes</h1>
          ...
          <div style={{ display: 'flex', gap: 32, alignItems: 'flex-start', position: 'relative' }}>
            ...
            <div style={{ width: 320, background: '#fff', borderRadius: 12, boxShadow: '0 1px 4px #0001', border: '1px solid #E5E7EB', padding: 18, minHeight: 600 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                {showMore && (
                  <div style={{ fontSize: 13, color: '#6B7280', padding: '4px 0' }}>+ {filtered.length - 10} more</div>
                )}
              </div>
            </div>
            {/* Center: Details */}
              ...
              <div style={{ flex: 1, background: '#fff', borderRadius: 12, boxShadow: '0 1px 4px #0001', border: '1px solid #E5E7EB', padding: 32, minWidth: 350, position: 'relative' }}>
                ...
                {showComments && (
                  <div style={{
                    ...
                    {/* Group comments by PrintStrike ID */}
                    {managerRecords && managerRecords.length > 0 ? (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                        {managerRecords.map((strike, idx) => (
                          <div key={strike._id || idx} style={{ marginBottom: 8 }}>
                            <div style={{ fontWeight: 700, color: '#1976d2', fontSize: 16, marginBottom: 6 }}>
                              {strike.season || strike._id || 'Print Strike'}
                            </div>
                            {strike.comments && strike.comments.length > 0 ? (
                              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                {strike.comments.map((c, cidx) => {
                              ...
                              <div style={{ color: '#888', fontSize: 15 }}>No comments for this Print Strike.</div>
                            )}
                          </div>
                        ))}
                      </div>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    {(selectedRecord && (selectedRecord.file || selectedRecord.s3Key || selectedRecord.imageUrl || selectedRecord.image)) ? (
                      <div style={{ position: 'relative', width: 320, height: 400, border: '1px solid #E5E7EB', borderRadius: 10, background: '#F9FAFB', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <img
                          src={selectedRecord.imageUrl || `http://localhost:3001/api/printstrike/${selectedRecord._id}/image`}
                  ...
                  <div style={{ flex: 1, marginLeft: 24 }}>
                    <div style={{ fontWeight: 600, marginBottom: 8 }}>Print Strike Details</div>
  ...
  if (loading) {
    return (
      <div style={{ background: '#F7F8FA', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <h2 style={{ fontWeight: 700, fontSize: 22, margin: '16px 0 8px 0' }}>Loading Print Strikes...</h2>
      </div>
    );
  }
  if (!managerGroups || managerGroups.length === 0) {
    return (
      <div style={{ background: '#F7F8FA', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <h2 style={{ fontWeight: 700, fontSize: 22, margin: '16px 0 8px 0' }}>No Print Strikes Found</h2>
        <p style={{ color: '#888', fontSize: 16 }}>No Print Strike data available. Please check your database or upload new records.</p>
      </div>
    );
  }
    // Search and sort for manager cards (logic only)
    ...
    return (
      <div style={{ background: '#F7F8FA', minHeight: '100vh', padding: 0, paddingTop: 80 }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 0 0 0', position: 'relative' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 0 }}>Print Strikes</h1>
          </div>
          <p style={{ color: '#6B7280', fontSize: 16, marginBottom: 24 }}>Track the status and progress of the Print Strikes</p>
